...
/**
 * Cubre vía la API real (routes/api.php) los dos endpoints que la app
...
class DriverHubTransferApiTest extends TestCase
{
    ...
    public function test_hub_driver_completes_collection_and_hub_reception_through_the_real_api(): void
    {
        ...
        // 4. Confirmar recepción en HUB.
        $this->postJson('/api/driver/scan/hub-reception', [
            'tracking_number' => $package->tracking_number,
        ], $headers)
            ->assertOk()
            ->assertJsonPath('package.current_status', \App\Models\Package::STATUS_EN_HUB);

        $package->refresh();
        $this->assertSame(\App\Models\Package::STATUS_EN_HUB, $package->current_status);

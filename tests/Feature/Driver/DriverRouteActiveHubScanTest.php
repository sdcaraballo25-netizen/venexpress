...
class DriverRouteActiveHubScanTest extends TestCase
{
    ...
    public function test_hub_distribution_route_reports_departure_then_arrival(): void
    {
        ...
        // El modelo de resolución logística (Fase 4/5B-1) ya no cuenta
        // paquetes pendientes por texto de ciudad/estado: exige
        // destination_warehouse_id + destination_resolution_status
        // resueltos. Se crea la cobertura del almacén para ese
        // destino y se deja que LogisticsResolutionService resuelva
        // el paquete — no se asume ni se escribe a mano qué almacén
        // "debería" ser.
        ...
        $resolution = app(LogisticsResolutionService::class)->resolveForPackage($package);
        $package->update([
            'destination_warehouse_id' => $resolution->warehouseId,
            'destination_resolution_status' => $resolution->status,
        ]);

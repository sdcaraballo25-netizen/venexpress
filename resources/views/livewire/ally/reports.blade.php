<div class="min-h-screen space-y-8 font-sans">
    ...
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-[#6B6B66]">
                Tendencias de {{ $ally->business_name }} entre
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5">
        ...
        <div class="bg-white rounded-2xl border border-[#E5E5E0] p-5 shadow-sm">
            <p class="text-xs font-bold text-[#6B6B66] uppercase tracking-wider">Registradas</p>
            <p class="mt-2 text-2xl font-bold text-[#111111]">{{ $registeredCount }}</p>
        </div>

        <div class="bg-white rounded-2xl border border-[#E5E5E0] p-5 shadow-sm">
            <p class="text-xs font-bold text-[#6B6B66] uppercase tracking-wider">Entregadas</p>
    ...
    <div
        wire:ignore
        x-data="venexpressAllyReportsCharts(@js($chartLabels), @js($chartRegistered), @js($chartDelivered), @js($chartRevenue))"
        x-init="init()"
        class="grid grid-cols-1 lg:grid-cols-2 gap-5"
    >
        <div class="bg-white rounded-2xl border border-[#E5E5E0] p-6 shadow-sm">
            <p class="text-xs font-bold text-[#6B6B66] uppercase tracking-wider mb-4">Guías por día</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div class="bg-white rounded-2xl border border-[#E5E5E0] shadow-sm overflow-hidden">
            <div class="p-5 border-b border-[#E5E5E0]">
                <p class="text-xs font-bold text-[#6B6B66] uppercase tracking-wider">Top ciudades de destino</p>
            </div>

            <div class="divide-y divide-[#E5E5E0]">
                @forelse ($topCities as $city)
                    <div class="flex items-center justify-between px-5 py-3">
                        <span class="text-sm font-medium text-[#111111]">{{ $city->destination_city }}, {{ $city->destination_state }}</span>
                        <span class="text-sm font-bold text-[#111111]">{{ $city->total }}</span>
                    </div>
                @empty
                    <p class="px-5 py-6 text-sm text-[#6B6B66]">Sin guías registradas en este período.</p>
                @endforelse
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-[#E5E5E0] shadow-sm overflow-hidden">
        <div class="p-5 border-b border-[#E5E5E0]">
            <p class="text-xs font-bold text-[#6B6B66] uppercase tracking-wider">
                Guías registradas en el período, por estado actual
            </p>
        </div>
...
<script>
    function venexpressAllyReportsCharts(labels, registered, delivered, revenue) {
        return {
            packagesChart: null,
            revenueChart: null,
            ...
            init() {
                ...
                Livewire.on('ally-reports-updated', ({ labels, registered, delivered, revenue }) => {

...
<div class="space-y-6">
    ...
    <div>
        ...
        <h1 class="mt-1 font-display text-2xl font-bold text-[#111111]">
    ...
    @if ($warehouse)
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="bg-white rounded-2xl border border-[#E5E5E0] p-6 shadow-sm">
                <div class="flex justify-between items-start">
                    <p class="text-xs font-bold text-[#6B6B66] uppercase tracking-wider">Por llegar</p>
                ...
                <p class="font-display text-3xl font-bold mt-4 text-[#111111]">{{ $pendingStops->count() }}</p>
                <p class="text-xs text-[#6B6B66] mt-2">Paradas todavía no confirmadas</p>
            </div>

            <div class="bg-white rounded-2xl border border-[#E5E5E0] p-6 shadow-sm">
                <div class="flex justify-between items-start">
                    <p class="text-xs font-bold text-[#6B6B66] uppercase tracking-wider">Recibidas</p>
                ...
                <p class="font-display text-3xl font-bold mt-4 text-[#111111]">{{ $visitedStops->count() }}</p>
                <p class="text-xs text-[#6B6B66] mt-2">Paradas confirmadas en tu almacén</p>
            </div>
        ...
        {{-- =========================================================
             LECTOR QR
        ========================================================== --}}
        <div id="escanear" class="bg-white border border-[#E5E5E0] rounded-2xl shadow-sm p-6 scroll-mt-24">
            <div class="flex items-center gap-3 mb-1">
                ...
                <h2 class="font-semibold text-[#111111]">Lector QR</h2>
            </div>
            <p class="text-xs text-[#6B6B66] mb-4">
        ...
        {{-- =========================================================
             ESCANEAR LLEGADA
        ========================================================== --}}
        <div class="bg-white border border-[#E5E5E0] rounded-2xl shadow-sm p-6">
            <div class="flex items-center gap-3 mb-1">
                <div class="p-2 bg-purple-50 rounded-lg text-purple-700">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <h2 class="font-semibold text-[#111111]">Escanear llegada de paquete</h2>
            </div>
            <p class="text-xs text-[#6B6B66] mb-4">
            ...
            <form wire:submit="scanArrival" class="flex flex-col sm:flex-row gap-3">
                <input
                    ...
                    autofocus
                    class="flex-1 rounded-xl border border-[#E5E5E0] px-4 py-3 text-sm focus:border-purple-900 focus:ring-purple-900"
                >
                <button
                    type="submit"
        ...
        <div class="bg-white border border-[#E5E5E0] rounded-2xl shadow-sm p-6">
            <div class="flex items-center gap-3 mb-1">
                ...
                <h2 class="font-semibold text-[#111111]">Despachar paquete</h2>
            </div>
            <p class="text-xs text-[#6B6B66] mb-4">
            ...
            <form wire:submit="searchDispatch" class="flex flex-col sm:flex-row gap-3">
                <input
                    ...
                    placeholder="Ej. VEN-2026-000123"
                    class="flex-1 rounded-xl border border-[#E5E5E0] px-4 py-3 text-sm focus:border-blue-900 focus:ring-blue-900"
                >
                <button
                    type="submit"
                ...
                <div class="mt-5 pt-5 border-t border-[#E5E5E0]">
                    ...
                    @if ($dispatchPackage->requires_delivery)
                        ...
                        @forelse ($this->availableDeliveryRoutes as $route)
                            <div class="flex items-center justify-between gap-3 rounded-xl border border-[#E5E5E0] px-4 py-3 mb-2">
                                <div>
                                    <p class="text-sm font-medium text-[#111111]">{{ $route->driver?->user?->name ?? 'Repartidor' }}</p>
                                    <p class="text-xs text-[#6B6B66]">{{ $route->name }} · {{ $route->city }}</p>
                                </div>
                                <button
                        ...
                        @empty
                            <p class="text-xs text-[#B8B8B2]">No hay repartidores con una ruta en curso hacia {{ $dispatchPackage->destination_city }} en este momento.</p>
                        @endforelse
        ...
        <div class="bg-white border border-[#E5E5E0] rounded-2xl shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-[#E5E5E0]">
                <h2 class="font-semibold text-[#111111]">Por llegar ({{ $pendingStops->count() }})</h2>
            </div>
            ...
            <div class="divide-y divide-[#F0F0EC]">
                ...
                @empty
                    <div class="px-6 py-12 text-center">
                        ...
                        <p class="font-semibold text-[#111111]">No hay paradas pendientes</p>
                        <p class="text-sm text-[#6B6B66] mt-1">Aparecerán aquí cuando una ruta de distribución tenga a tu almacén como destino.</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white border border-[#E5E5E0] rounded-2xl shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-[#E5E5E0]">
                <h2 class="font-semibold text-[#111111]">Recibidas recientemente ({{ $visitedStops->count() }})</h2>
            </div>
            ...
            <div class="divide-y divide-[#F0F0EC]">
                ...
                @empty
                    <div class="px-6 py-12 text-center">
                        ...
                        <p class="font-semibold text-[#111111]">Todavía no se ha recibido ninguna parada</p>
                        <p class="text-sm text-[#6B6B66] mt-1">Las paradas confirmadas aparecerán aquí.</p>
                    </div>
                @endforelse
            </div>
        </div>
    ...
    @else
        ...
        <div class="bg-white border border-[#E5E5E0] rounded-2xl shadow-sm p-12 text-center">
            ...
            <p class="font-semibold text-[#111111]">Sin almacén asignado</p>
            <p class="text-sm text-[#6B6B66] mt-1">Contacta a un administrador para que te asigne uno.</p>
        </div>

<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="inline-flex rounded-2xl border border-[#E2E8F0] bg-white p-1">
            <button
                ...
                class="px-4 py-1.5 text-sm font-medium rounded-xl transition
                    {{ $period === 'today' ? 'bg-blue-900 text-white' : 'text-slate-500 hover:text-[#0F172A]' }}"
            >
                Hoy
            </button>
            <button
                ...
                class="px-4 py-1.5 text-sm font-medium rounded-xl transition
                    {{ $period === 'week' ? 'bg-blue-900 text-white' : 'text-slate-500 hover:text-[#0F172A]' }}"
            >
                Esta semana
            </button>
            <button
                ...
                class="px-4 py-1.5 text-sm font-medium rounded-xl transition
                    {{ $period === 'month' ? 'bg-blue-900 text-white' : 'text-slate-500 hover:text-[#0F172A]' }}"
            >
                Este mes
            </button>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="rounded-2xl border border-[#E2E8F0] bg-white p-5">
            <p class="text-sm text-slate-500">Guías procesadas</p>
            <p class="mt-2 font-display text-2xl font-semibold text-[#0F172A]">
                {{ $processedCount }}
            </p>
        </div>
        ...
        <div class="rounded-2xl border border-[#E2E8F0] bg-white p-5">
            <p class="text-sm text-slate-500">Comisión del período</p>
            <p class="mt-2 font-display text-2xl font-semibold text-[#0F172A]">
        ...
        <div class="rounded-2xl border border-[#E2E8F0] bg-blue-900 p-5">
            <p class="text-sm text-blue-200">Saldo total por comisiones</p>
                    ...
                    @foreach ($byPaymentMethod as $row)
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-600">
                                ...
                                <span class="text-slate-400">({{ $row->guides }})</span>
                            </span>
        ...
        <div class="rounded-2xl border border-[#E2E8F0] bg-white p-5">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex items-center justify-between">
                    <span class="text-slate-600">Pendiente de liquidar</span>
                    <span class="font-medium text-amber-600">
                        ${{ number_format($codPendingUsd, 2) }}
                    </span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-slate-600">Liquidado</span>
